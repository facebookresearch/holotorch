...
from holotorch.utils.Dimensions import BTCHW_E, TensorDimension
...
class SLM_Complex_Polar(Modulator_Container):
    ...
    def __init__(self,
        ...
        extended_dimension = BTCHW_E.from_BTCHW(tensor_dimension)
    ...
    def reset_values(self, noise_variance = 0.1):
        ...
        complex_values =  torch.ones_like(self.values.data_tensor[...,0]) * torch.exp(1j * noise_variance * torch.rand_like(self.values.data_tensor[...,0]) )
        magnitude = torch.abs(complex_values)
        phase     = torch.angle(complex_values)
        
        
        new_values = torch.ones_like(self.values.data_tensor)
        new_values[..., 0] = magnitude
        new_values[..., 1] = phase
    ...
    def forward(self,
        ...
        values, scale1 = super().forward()
        magnitude   = values[...,0]        
        phase       = values[...,1]        

        slm_field = magnitude * torch.exp(1j*phase)
